...
def get_object_url(
    instance: Any, notification: Notification, request: HttpRequest
) -> str | None:
    """
    ...
    if hasattr(instance, "get_url_for_notifications"):
        return instance.get_url_for_notifications(notification, request)  # type: ignore[no-any-return]
    elif hasattr(instance, "get_absolute_url"):
        return instance.get_absolute_url()  # type: ignore[no-any-return]
    return None


def get_num_to_fetch(request: HttpRequest) -> int:
...
def get_notification_list(
    request: HttpRequest, method_name: str = "all"
) -> list[dict[str, Any]]:
    num_to_fetch = get_num_to_fetch(request)
    notification_list = []
    notification_objects = Notification.objects.filter(recipient=request.user.id)  # type: ignore[union-attr]
